...
@Component({
  ...
  imports: [
    ...
    MatRadioModule, MatAccordion, MatExpansionModule, MatDividerModule, AddCustomGroupsComponent,MatInputModule,
    MatTooltipModule
  ],
  styleUrls: ['./add-item.component.css'],
...
export class DialogContent {
  ...
  processCustomItems(formsGroup: any,cgName:string){
    formsGroup.forEach((group:any)=>{
        ...
        let customItem:any={
          viewValue:`${group?.variantName}${Cparent!==null?(` [parent-> ${Cparent}]`):""}`,
          value:customID,
        }
    ...
    switch (groupIndex) {
      case 1:
        if(formData?.variantForms){
          ...
          let groupedVariants1:any = this.groupByParentCg(formData?.variantForms);
